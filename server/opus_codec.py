...
OPUS_APPLICATION_AUDIO = 2049  # â† NEW: Cleaner for mixed voice (less formant boost)
OPUS_OK = 0
...
OPUS_SET_VBR_REQUEST = 4006  # â† NEW: Explicit VBR (variable bitrate) for better quality
OPUS_SET_VBR_CONSTRAINT_REQUEST = 4007  # â† NEW: Constrained VBR (trade-off for stability)
...
class OpusCodec:
    ...
    def __init__(
        ...
        channels=1,
        frame_size=320,
        enable_fec=True,  # â† IMPROVED: Always on for VoIP
        packet_loss_perc=15,  # â† IMPROVED: 15% sim for Wi-Fi robustness (was 10)
        bitrate=48000,  # â† IMPROVED: 48kbit/s for natural voice (was 16k/32k)
        complexity=10,  # â† IMPROVED: Higher for fewer artifacts (was 10)
        enable_dtx=True,
        application=OPUS_APPLICATION_AUDIO,  # â† NEW: AUDIO mode for less "noisy" processing
        create_encoder=True,
        create_decoder=True,
    ):
        ...
        if create_encoder:
            ...
            # â† NEW: Enable VBR for dynamic quality (saves bits in silence, boosts speech)
            ...
            # self._set_encoder_ctl_int(OPUS_SET_DTX_REQUEST, 1 if enable_dtx else 0)  # â† TEMP DISABLE
    ...
    def decode(self, opus_bytes):
        ...
        if opus_bytes:
            ...
            # â† IMPROVED: PLC with fade (Opus handles this internally, but we can post-process if needed)
